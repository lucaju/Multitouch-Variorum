package views.readerWindow.reader {
	...
	import views.readerWindow.reader.variantFloatBox.VariantFloatBox;
	...
	public class Reader extends Sprite {
		...
		protected var w							:int = 335;
		protected var h							:int = 440;
			
		protected var marginW					:uint = 0 //5;
		protected var marginH					:uint = 0 //3;
		...
		public function init(data:XML):void {
			...
			//border
			border = new Shape();
			border.graphics.lineStyle(1,0x444444,.4);
			border.graphics.lineTo(0,h);
			border.graphics.moveTo(w,0);
			border.graphics.lineTo(w,h);
		 ...
		 * 
		 */
		protected function getVariableSpanByID(value:int):SpanElement {
			var spans:Array = readerFlow.getElementsByTypeName("span");
			
			for each (var span:SpanElement in spans) {
				if (span.id == value.toString()) {
					return span;
				}
			}
		...
		public function mouseDown(e:FlowElementMouseEvent):void {
			...
			*/
			...
			// dispatch event
			this.dispatchEvent(new ReaderEvent(ReaderEvent.VARIABLE_SELECTED,span.id));
		}
		 ...
		 * 
		 */
		public function updateVariable(variableID:int, variationText:String):void {
			var variableSpan:SpanElement = getVariableSpanByID(variableID);
			variableSpan.replaceText(0,variableSpan.textLength,variationText);
			readerFlow.flowComposer.updateAllControllers();
		}
		 ...
		 * 
		 */
		public function scrollToVariable(variableID:int):void {
			
			//get element
			var variableSpan:SpanElement = getVariableSpanByID(variableID);
			
			//get Absolute start
			var absolueStart:int = variableSpan.getAbsoluteStart();
		 ...
		 * 
		 */
		public function scrollToLine(value:int):void {
			var lineNumber:int = value - 1; //lines are base 0;
			var line:TextFlowLine = readerFlow.flowComposer.getLineAt(lineNumber);
